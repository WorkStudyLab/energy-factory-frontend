...
const ShadcnTestPage: React.FC = () => {
  ...
  const renderCheckbox = (checkbox: any) => {
    ...
    return (
      <div key={checkbox.name} className="flex items-center space-x-2 p-4 border rounded-lg hover:bg-gray-50">
        <Checkbox
          id={checkbox.id}
          checked={checkboxStates[checkbox.id] || checkbox.defaultChecked || false}
      ...
      <div className="max-w-6xl mx-auto px-4">
        ...
        <div className="bg-white rounded-xl shadow-lg p-8">
          <h2 className="text-2xl font-bold text-gray-900 mb-6">
            {tabs.find(tab => tab.id === activeTab)?.icon} {tabs.find(tab => tab.id === activeTab)?.label} 테스트
          </h2>
          
          <div className="grid gap-4">
            {activeTab === 'buttons' && shadcnComponents.buttons.map(renderButton)}
